...
ros::Publisher encoder_imu_pub;		//创建融合消息发布者 
...
void callback(const sensor_msgs::Imu::ConstPtr& imu_data,const nav_msgs::Odometry::ConstPtr& speed)
{
        ...
        if(flag==false)
        {
			...
			odom_trans.transform.rotation = tf::createQuaternionMsgFromYaw(theta);		//将角度转换成旋转矩阵
...
{
	ros::init(argc,argv,"encoder_imu_mix");		//创建节点 节点名 encoder_imu_mix
	ros::NodeHandle nh;		//创建句柄 
	
	message_filters::Subscriber<sensor_msgs::Imu> imu_sub(nh,"/imu_data",100);		//imu消息订阅者  订阅话题/imu_data  缓存队列100	
	message_filters::Subscriber<nav_msgs::Odometry> encoder_sub(nh,"/encoder",20);	//encoder消息订阅者  订阅话题/encoder  缓存队列20 
	encoder_imu_pub = nh.advertise<nav_msgs::Odometry>("/encoder_imu_odom",10);		//创建发布后融合的消息的节点 发送缓存队列10 
